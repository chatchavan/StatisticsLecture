...
# Re-analyzing mobile phone usage at meal time (Moser et al., 2016)

**Note:** The dataset doesn't contain variable (f) How many individuals were included in the mealtime drawing?
```{r}
# preprocess data (need to run only once)
...
```{r}
meal_data <- read_csv("../data/TechAtMeals_CHI2016/meal_data.csv" )
meal_code <- read_csv("../data/TechAtMeals_CHI2016/meal_code.csv")
print(meal_code)
```
...
```{r}
# Note the last parameter: we use the full set of data
m_paper <- update(lm_all, . ~ . - question_11 - question_12 - housemates - gender, data = meal_data)
summary(m_paper)
```

Here, we have a reasonable residual Df of (`r summary(m_paper)$fstatistic[2]`), which is much closer to the number of rows in the data (`r nrow(meal_data)` rows).

# Your turn
Try updating model and determine which variables to include based on AIC.
...
# Hint: extractAIC(update(lm_all, . ~ .  - #var1# - #var2# , data = meal_data))
```
