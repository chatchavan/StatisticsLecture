...
renderPlot({
  ...
  ggplot(data1, aes(x = x, y = y)) +
    ...
    geom_segment(data = errLines, aes(x = x, y = y, xend = xend, yend = yend),
                 linetype = "solid", color = "red", size = input$dashedLineSize) +
...
inputPanel(
  ...
  list(
    renderUI({
      ...
      withMathJax(sprintf("\\(SS_M\\) = %.2f", SS_M))
    })
  )
)
...
renderUI({
  ...
  withMathJax(sprintf("\\(F = \\frac{MS_M}{MS_R} = \\frac{%.2f}{%.2f} = %.2f \\)",
                      MS_M, MS_R, FValue))
})
...
#Interactive 4
$F$-distribution. The red line is the 95th percentile. This means that only 5% of F values fall to the right of the red line.
```{r, echo=FALSE}

inputPanel(
  sliderInput("df1", label = "\\(df_1\\):",
              min = 1, max = 10, value = 1),

  sliderInput("df2", label = "\\(df_2\\):",
...
renderPlot({
  df1 <- input$df1
  df2 <- input$df2
  FValues <- rf(10000, df1, df2)
  
  critF <- qf(.95, df1=df1, df2=df2) 
  
  ggplot(as.data.frame(FValues), aes(x = FValues)) +
    geom_blank() + 
    stat_function(fun = df, args = list(df1 = df1, df2 = df2), size = input$lineSize) +
    geom_vline(xintercept=critF, color = "red", size = input$lineSize) +
...
# 
# x <- seq(0,10, 0.1)
# curve(df(x, df1 = df1, df2 = df2), from = 0, to = 20)
# 
# ggplot(as.data.frame(FValues), aes(x = FValues)) + geom_density() + stat_function(fun = df, args = list(x = seq(0,10, 0.1), df1 = df1, df2 = df2),
#     color = "blue", lty = "solid")
```
...
#Interactive 5
```{r, echo=FALSE}
data2 <- data.frame(x = sample(1:30, 50, replace = TRUE), y = sample(1:120, 50, replace = TRUE))
inputPanel(
  list(
    HTML("Black line:"),
    sliderInput("blk_b0", label = "\\(\\beta_0\\):",
    ...
    # show sum of square error
    renderUI({
      errLines <- calcError(data2, input$blk_b0, input$blk_b1)
      SS_R <- sum((errLines$yend - errLines$y)^2)
      withMathJax(sprintf("\\(SS_R\\) = %.2f\n \\(MS_R \\)= %.2f", SS_R, SS_R / (N - 1)))
    })
  ...
  list(
    ...
    # show sum of square error
    renderUI({
      errLines <- calcError(data2, input$g_b0, 0)
      SS_T <- sum((errLines$yend - errLines$y)^2)
      withMathJax(sprintf("\\(SS_T\\) = %.2f\n", SS_T))
    })
  ),
...
renderUI({
  ...
  g_errLines <- calcError(data2, input$g_b0, 0)
  SS_T <- sum((g_errLines$yend - g_errLines$y)^2)
...
plotNull <- renderPlot({
      errLines_g <- calcError(data2, input$g_b0, 0)
